...
describe("recover tests", () => {
	...
	before(async () => {
		try {
			...
			await agent.post("/api/singup").send(testUser);
		} catch (err) {
			throw err;
		}
	});
	...
	describe("Post: /recover", () => {
		it("The recover must be saved in the database correctly and the mail must be sent correctly", async () => {
			try {
				...
				expect(new Date(recover.expire).getTime()).to.be.greaterThan(
					new Date().getTime()
				);
				...
				expect(emailNoti).to.have.been.calledWith(
					sinon.match({html: sinon.match(new RegExp(recover._id, "gi"))})
				);
			} catch (err) {
				throw err;
			}
	...
	describe("Get: /recover/:id - success", () => {
		it("if recover exist and si not expire return recover form", (done) => {
			agent.get(`/api/recover/${recoverId}`).end((err, res) => {
	...
	describe("Put: /recover/:id - success", () => {
		...
		it("Password must be update", (done) => {
			agent
				.put(`/api/recover/${recoverId}`)
	...
	describe("Get: /recover/:id - unsuccess", () => {
		...
		it("if recover exist but is expire", async () => {
			try {
				...
				await agent.get(`/api/recover/${rid}`).then((res) => {
					expect(res.type).to.equal("text/html");
					expect(res.text).to.equal("Link expires, get a new one");
				});
			} catch (err) {
